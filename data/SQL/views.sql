...
IF OBJECT_ID('dbo.v_parkinglot_summary') IS NOT NULL
    DROP VIEW dbo.v_parkinglot_summary
GO

CREATE view v_parkinglot_summary
AS
SELECT lot.id
     , lot.buken_id
     , bk.bk_name as name
     , lot.lng
     , lot.lat
	 , lot.is_existed_contractor_allowed
	 , lot.is_new_contractor_allowed
	 , lot.free_end_date
     , (SELECT COUNT(1) FROM [areaparking].[dbo].[ap_parking_position] pos WHERE pos.parking_lot_id = lot.id) AS position_count
     , (SELECT COUNT(1) FROM [fk5dtsql].[dbo].[ky_kosinkai] ky WHERE ky.bk_no = lot.buken_id AND ky.ky_start_ymd <= GETDATE() AND ky.ky_end_ymd >= GETDATE()) AS contract_count
	 , lot.created_date
	 , lot.updated_date
	 , lot.is_deleted
	 , lot.deleted_date
  FROM [areaparking].[dbo].[ap_parking_lot] lot
  JOIN [areaparking].[dbo].[bk_mst] bk ON bk.bk_no = lot.buken_id
